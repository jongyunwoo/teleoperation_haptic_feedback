...
class DistanceOverlay:
    ...
    def __init__(self, focal_len_px: float = 615.0, min_dist: float = 0.01, max_dist: float = 0.10):
        self.focal_len_px = focal_len_px
        self.min_dist = min_dist
        self.max_dist = max_dist

    def compute_mask_centroid(self, mask: np.ndarray) -> tuple:
    ...
    def compute_object_distance_simple(
        self,
        center1: tuple,
        center2: tuple,
        ...
        z1 = depth_image[v1, u1]
        z2 = depth_image[v2, u2]
        if z1 == 0 or z2 == 0:
            return None
        z1_m, z2_m = z1 / 1000.0, z2 / 1000.0
        z_avg = (z1_m + z2_m) / 2.0
        d_pixel = np.hypot(u2 - u1, v2 - v1)
        return (d_pixel * z_avg) / self.focal_len_px
    ...
    def overlay_mask_with_color(
        ...
        # map distance to 0–1
        norm = np.clip((distance - self.min_dist) / (self.max_dist - self.min_dist), 0, 1)
        # color: red→green
        color = (0, int(norm * 255), int((1 - norm) * 255))  # BGR
        ...
        return cv2.addWeighted(image, 1.0, overlay, 0.5, 0)
